...
def printPunch(p1, p2, p3, img=None, color=(255, 0, 255), scale=5):
    """
    ...
    if img is not None:
        ...
        cv2.putText(img, 'SOCO', (x2 - 50, y2 + 50),
                    cv2.FONT_HERSHEY_PLAIN, 2, color, max(1, scale // 5))
    return img

detector.printPunch = printPunch
...
while True:
    ...
    if lmList:
        ...
        length1, img, info = detector.findDistance(lmList[12][0:2],
                                                   lmList[16][0:2],
                                                   img,
                                                   color=(255, 130, 0),
                                                   scale=10)
        ...
        isCloseAngle51 = detector.angleCheck(myAngle=angle1,
                                             targetAngle=190,
                                             offset=15)

        angleArmHip, img = detector.findAngle(lmList[24][0:2],
                                              lmList[12][0:2],
                                              lmList[16][0:2],
                                              img=img,
                                              color=(0, 0, 255),
        ...
        if isCloseAngle50:
            img = detector.printPunch(lmList[11][0:2],
                                      ...
                                      color=(0, 0, 255),
                                      scale=10)
        if isCloseAngle51 and isAngleArmHip:
